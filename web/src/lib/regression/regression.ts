 ...
 */
function determinationCoefficient(data: [number, number][], results: [number, number][]) {
 ...
 */
function gaussianElimination(input: number[][], order: number) {
  const matrix = input;
 ...
 */
const methods = {
  linear(data: [number, number][], options: RegressionOptions) {
    const sum = [0, 0, 0, 0, 0];
    let len = 0;
    ...
    const points = data.map((point: [number, number]) => predict(point[0]));
  ...
  exponential(data: [number, number][], options: RegressionOptions) {
  ...
  logarithmic(data: [number, number][], options: RegressionOptions) {
    const sum = [0, 0, 0, 0];
  ...
  power(data: [number, number][], options: RegressionOptions) {
    const sum = [0, 0, 0, 0, 0];
    const len = data.length;
  ...
  polynomial(data: [number, number][], options: RegressionOptions) {
    const lhs = [];
    const rhs = [];
    let a = 0;
    let b = 0;
    const len = data.length;
    const k = (options.order ?? DEFAULT_OPTIONS.order) + 1;
...
export const regression = {
  ...
  logarithmic: methods.logarithmic,
  power: methods.power,
  polynomial: methods.polynomial
};
