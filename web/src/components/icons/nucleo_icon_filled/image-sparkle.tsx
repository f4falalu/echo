...
function 18px_imageSparkle(props: iconProps) {
	const fill = props.fill || 'currentColor';
	const secondaryfill = props.secondaryfill || fill;
	const strokewidth = props.strokewidth || 1;
	const width = props.width || '1em';
	const height = props.height || '1em';
	const title = props.title || "18px image sparkle";

	return (
		<svg height={height} width={width} viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
	<title>{title}</title>
	<g fill={fill}>
		<path d="M13.444,9.056c-1.038-1.039-2.851-1.039-3.889,0L3.678,14.934c.31,.198,.677,.316,1.072,.316H13.25c1.105,0,2-.896,2-2v-2.389l-1.806-1.806Z" fill={secondaryfill}/>
...
export default 18px_imageSparkle;