...
function 18px_calendarDate(props: iconProps) {
	const fill = props.fill || 'currentColor';
	const secondaryfill = props.secondaryfill || fill;
	const strokewidth = props.strokewidth || 1;
	const width = props.width || '1em';
	const height = props.height || '1em';
	const title = props.title || "18px calendar date";
	...
	<g fill={fill}>
		...
		<path d="M9.25,13.5c-.414,0-.75-.336-.75-.75v-2.431c-.164,.067-.344,.122-.54,.16-.4,.075-.8-.189-.877-.596-.078-.407,.188-.8,.596-.878,.604-.116,.911-.613,.924-.634,.175-.289,.525-.427,.846-.34,.324,.09,.552,.381,.552,.718v4c0,.414-.336,.75-.75,.75Z" fill={secondaryfill}/>
	</g>
</svg>
	);
};

export default 18px_calendarDate;