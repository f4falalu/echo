...
export const identityTypeEnum = pgEnum("identity_type_enum", [
	"user",
	"team",
	"organization",
]);
export const messageFeedbackEnum = pgEnum("message_feedback_enum", [
	"positive",
	"negative",
]);
...
export const slackSharingPermissionEnum = pgEnum(
	...
	["shareWithWorkspace", "shareWithChannel", "noSharing"],
);
...
export const teams = pgTable(
	"teams",
	{
		...
		name: text().notNull(),
		organizationId: uuid("organization_id").notNull(),
		sharingSetting: sharingSettingEnum("sharing_setting")
			.default("none")
			.notNull(),
...
export const datasetColumns = pgTable(
	...
	(table) => [
		unique("unique_dataset_column_name").on(table.datasetId, table.name),
	],
);
	...
	(table) => [
		...
		index("pgroonga_content_index").using(
			...
			table.content
				.asc()
				.nullsLast()
				.op("pgroonga_text_full_text_search_ops_v2"),
		),
	],
);
...
export const datasetPermissions = pgTable(
	...
	(table) => [
		...
		).on(table.datasetId, table.permissionId, table.permissionType),
...
export const datasets = pgTable(
	...
	(table) => [
		...
		unique("datasets_database_name_data_source_id_key").on(
			table.databaseName,
			table.dataSourceId,
		),
	],
...
export const messagesToFiles = pgTable(
	...
	(table) => [
		...
		unique("messages_to_files_message_id_file_id_key").on(
			table.messageId,
			table.fileId,
		),
	],
...
export const chats = pgTable(
	"chats",
	{
		...
		slackChatAuthorization: slackChatAuthorizationEnum(
	...
	(table) => [
		...
		index("chats_created_by_idx").using(
			"btree",
			table.createdBy.asc().nullsLast().op("uuid_ops"),
		),
...
export const organizations = pgTable(
	"organizations",
	{
		...
		restrictNewUserInvitations: boolean("restrict_new_user_invitations")
			.default(false)
			.notNull(),
		defaultRole: userOrganizationRoleEnum("default_role")
...
export const databaseMetadata = pgTable(
	...
	(table) => [
		...
		unique("database_metadata_data_source_id_name_key").on(
			table.dataSourceId,
			table.name,
		),
...
export const slackIntegrations = pgTable(
	"slack_integrations",
	{
		...
		defaultSharingPermissions: slackSharingPermissionEnum(
	...
	(table) => [
		...
		unique("slack_integrations_org_team_key").on(
			table.organizationId,
			table.teamId,
		),
