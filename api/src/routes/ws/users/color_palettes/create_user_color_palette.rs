...
pub struct CreateUserColorPaletteRequest {
    pub color_palette: Vec<String>,
}

pub async fn create_user_color_palette(
    user: &User,
    req: CreateUserColorPaletteRequest,
) -> Result<()> {
    let user_subscription = format!("{}", &user.id);

    let create_user_color_palette_res = match create_user_color_palette_handler(&user.id, req).await
    {
        Ok(res) => res,
        Err(e) => {
...
async fn create_user_color_palette_handler(
    user_id: &Uuid,
    req: CreateUserColorPaletteRequest,
    ...
    if let Some(color_palettes) = &mut user_config.color_palettes {
        color_palettes.push(req.color_palette);
    } else {
    ...
    tokio::spawn(async move {
        match update_user_color_palette_handler(update_user_id, update_user_config).await {
            Ok(_) => (),
            Err(e) => {
...
async fn update_user_color_palette_handler(user_id: Uuid, user_config: UserConfig) -> Result<()> {
    let mut conn = match get_pg_pool().get().await {
        Ok(conn) => conn,
        Err(e) => return Err(anyhow!("Error getting connection: {}", e)),
    };
