...
pub struct DeleteUserColorPaletteRequest {
    pub id: usize,
}

pub async fn delete_user_color_palette(
    user: &User,
    req: DeleteUserColorPaletteRequest,
) -> Result<()> {
    let user_subscription = format!("{}", &user.id);

    let create_user_color_palette_res = match delete_user_color_palette_handler(&user.id, req).await
    {
        Ok(res) => res,
        Err(e) => {
...
async fn delete_user_color_palette_handler(
    user_id: &Uuid,
    req: DeleteUserColorPaletteRequest,
    ...
    if let Some(color_palettes) = &mut user_config.color_palettes {
        color_palettes.remove(req.id);
    } else {
        user_config.color_palettes = Some(vec![]);
    }
    ...
    tokio::spawn(async move {
        match delete_user_color_palette_task(update_user_id, update_user_config).await {
            Ok(_) => (),
            Err(e) => {
...
async fn delete_user_color_palette_task(user_id: Uuid, user_config: UserConfig) -> Result<()> {
    let mut conn = match get_pg_pool().get().await {
        Ok(conn) => conn,
        Err(e) => return Err(anyhow!("Error getting connection: {}", e)),
    };
