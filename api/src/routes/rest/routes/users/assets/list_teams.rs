...
async fn list_teams_handler(user: User, user_id: Uuid) -> Result<Vec<TeamInfo>> {
    ...
    let teams = match teams::table
        ...
        .select((
            teams::id,
            teams::name,
            diesel::dsl::sql::<diesel::sql_types::BigInt>("COALESCE(count(teams_to_users.user_id), 0)"),
            diesel::dsl::sql::<diesel::sql_types::Bool>("teams_to_users.user_id IS NOT NULL"),
        ))
        .group_by((teams::id, teams::name, teams_to_users::user_id))
    ...
    Ok(teams
        .into_iter()
        .map(|(id, name, count, assigned)| TeamInfo {
            id,
            name,
            user_count: count,
            assigned,
