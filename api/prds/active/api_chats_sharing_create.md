...
pub async fn create_chat_sharing_rest_handler(
    ...
    match create_chat_sharing_handler(&id, &user.id, request.emails, request.role).await {
    ...
    for email in emails {
        ...
        match create_share_by_email(
            ...
            *user_id,
            ...
            Ok(_) => {
                tracing::info!("Created sharing permission for email: {} on chat: {}", email, chat_id);
            },
            Err(e) => {
...
- Test POST /chats/:id/sharing with invalid roles
...
- Ensure that only users with Owner or FullAccess permission can share chats
