---
title: Remove Dashboards from Collection REST Endpoint
author: Cascade
date: 2025-03-19
status: Draft
---

# Remove Dashboards from Collection REST Endpoint

## Problem Statement

Users need the ability to programmatically remove dashboards from collections via a REST API. Currently, this functionality is not available, limiting the ability to manage collections through the API.

## Goals

1. Create a REST endpoint to remove dashboards from a collection
...
**Endpoint:** `DELETE /collections/:id/dashboards`
...
- `200 OK` - Success
  ```json
  {
    "message": "Dashboards removed from collection successfully"
  }
  ```
...
3. Mark the specified dashboards as deleted in the `collections_to_assets` table

### File Changes

#### New Files

1. `libs/handlers/src/collections/remove_dashboards_from_collection_handler.rs`
```rust
...
/// Removes dashboards from a collection
///
/// # Arguments
///
/// * `collection_id` - The unique identifier of the collection
/// * `dashboard_ids` - Vector of dashboard IDs to remove from the collection
/// * `user_id` - The unique identifier of the user performing the action
///
/// # Returns
///
/// Ok(()) on success, or an error if the operation fails
pub async fn remove_dashboards_from_collection_handler(
    collection_id: &Uuid,
    ...
    info!(
        ...
        "Removing dashboards from collection"
    );
    ...
    let updated = diesel::update(collections_to_assets::table)
        ...
        .await
        .map_err(|e| {
            error!("Error removing dashboards from collection: {}", e);
            anyhow!("Database error: {}", e)
        })?;
    ...
    info!(
        ...
        "Successfully removed dashboards from collection"
    );
...
mod tests {
    ...
    #[tokio::test]
    async fn test_remove_dashboards_from_collection_handler() {
...
2. `src/routes/rest/routes/collections/remove_dashboards_from_collection.rs`
```rust
use axum::{
    extract::{Extension, Json, Path},
    http::StatusCode,
};
use handlers::collections::remove_dashboards_from_collection_handler;
...
pub struct RemoveDashboardsRequest {
    pub dashboard_ids: Vec<Uuid>,
}

/// REST handler for removing dashboards from a collection
///
/// # Arguments
///
/// * `user` - The authenticated user making the request
/// * `id` - The unique identifier of the collection
/// * `request` - The dashboard IDs to remove from the collection
///
/// # Returns
///
/// A success message on success, or an appropriate error response
pub async fn remove_dashboards_from_collection_rest_handler(
    Extension(user): Extension<AuthenticatedUser>,
    Path(id): Path<Uuid>,
    Json(request): Json<RemoveDashboardsRequest>,
) -> Result<ApiResponse<String>, (StatusCode, String)> {
    info!(
        collection_id = %id,
        user_id = %user.id,
        dashboard_count = request.dashboard_ids.len(),
        "Processing DELETE request to remove dashboards from collection"
    );

    match remove_dashboards_from_collection_handler(&id, request.dashboard_ids, &user.id).await {
        Ok(_) => Ok(ApiResponse::JsonData("Dashboards removed from collection successfully".to_string())),
        Err(e) => {
            tracing::error!("Error removing dashboards from collection: {}", e);
            ...
            Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to remove dashboards from collection: {}", e)))
        }
    }
}
```
...
1. Test the handler with mocked database connections
   - Test removing dashboards from a collection
   - Test error cases (collection not found, insufficient permissions)
   - Test removing dashboards that aren't in the collection
...
1. Test the endpoint with a test database
   - Create a collection and add dashboards to it
   - Remove dashboards from the collection
