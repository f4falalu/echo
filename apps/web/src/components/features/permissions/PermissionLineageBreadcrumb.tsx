...
const UserLineageItem: React.FC<LineageItemProps> = ({ name }) => {
  return <div className={linearItem({ clickable: false })}>{name}</div>;
};

const DatasetLineageItem: React.FC<LineageItemProps> = ({ name, id }) => {
  return (
    <Link to="/app/datasets/$datasetId" params={{ datasetId: id }}>
      <div className={linearItem({ clickable: true })}>{name}</div>
    </Link>
  );
};

const PermissionGroupLineageItem: React.FC<LineageItemProps> = ({ name }) => {
  return <div className={linearItem({ clickable: false })}>{name}</div>;
};

const DatasetGroupLineageItem: React.FC<LineageItemProps> = ({ name }) => {
  return <div className={linearItem({ clickable: false })}>{name}</div>;
};
