...
const ColorPickButton: React.FC<{
  ...
  return (
    <div className="flex h-7 w-full cursor-pointer">
...
}> = React.memo(({ color: colorProp, index, isFirst, isLast, onColorChange }) => {
    ...
    <ColorPicker
      ...
      popoverChildren={
        <div className="flex w-full items-center gap-2 border-t py-2">
          ...
          <Button block variant={'primary'} onClick={() => onColorChange(color, index)}>
            Save
          </Button>
        </div>
      }>
      <div
        className={cn(
          'flex-1 overflow-hidden transition-transform duration-150 hover:z-10 hover:scale-120 hover:shadow',
          isFirst && 'rounded-l',
          isLast && 'rounded-r'
        )}
        style={{ backgroundColor: color }}>
        <AppTooltip title={color} delayDuration={400} skipDelayDuration={500}>
          <div className="h-full w-full flex-1" style={{ backgroundColor: color }} />
        </AppTooltip>
      </div>
    </ColorPicker>
  );
