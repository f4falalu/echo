...
import { type ChartConfigProps, DEFAULT_TRENDLINE_CONFIG } from '@buster/server-shared/metrics';
...
export const EditTrendline: React.FC<{
  ...
  ({
    ...
    const onUpdateTrendlines = (trends: LoopTrendline[]) => {
      setTimeout(() => {
        onUpdateChartConfig({ trendlines: trends });
      }, 30);
    };

    const onDeleteTrendline = (id: string) => {
      setTrends((prev) => {
        return prev.filter((trend) => trend.id !== id);
      });
    };

    const onUpdateExistingTrendline = (trend: LoopTrendline) => {
      setTrends((prev) => {
