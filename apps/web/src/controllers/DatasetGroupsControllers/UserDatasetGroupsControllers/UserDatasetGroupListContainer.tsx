...
export const UserDatasetGroupListContainer: React.FC<{
  ...
  const { cannotQueryPermissionUsers, canQueryPermissionUsers } = useMemo(() => {
    const createDatasetGroupListItem = createListItem<BusterUserDatasetGroup>();
    const result: {
      ...
      (acc, datasetGroup) => {
        const user: BusterListRowItem<BusterUserDatasetGroup> = createDatasetGroupListItem({
          id: datasetGroup.id,
          data: datasetGroup,
          link: {
            to: '/app/settings/dataset-groups/$datasetGroupId',
            params: {
