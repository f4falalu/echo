...
import { PermissionAssignTeamRole } from '@/components/features/permissions';
import {
...
export const UserTeamsListContainer: React.FC<{
  ...
  const { managerTeams, memberTeams, notAMemberTeams } = useMemo(() => {
    const createTeamListItem = createListItem<BusterUserTeamListItem>();
    const result: {
      ...
      (acc, team) => {
        const teamItem: BusterListRowItem<BusterUserTeamListItem> = createTeamListItem({
          id: team.id,
          data: team,
          link: {
